...
Run: `cd /path/to/mcp-video-gen && uv sync --extra gcp`
Expected: installs google-auth and its dependencies

- [ ] **Step 3: Verify import works**

Run: `cd /path/to/mcp-video-gen && uv run --extra gcp python -c "import google.auth; print('ok')"`
Expected: `ok`

- [ ] **Step 4: Commit**

```bash
cd /path/to/mcp-video-gen
...
Run: `cd /path/to/mcp-video-gen && uv run --extra gcp python -c "from video_gen.providers.veo import VeoProvider; print('ok')"`
Expected: `ok`
...
Run: `cd /path/to/mcp-video-gen && uv run python -c "from video_gen.server import server; print('ok')"`
...
GCP_PROJECT_ID=logical-bird-240509 \
  npx @modelcontextprotocol/inspector uv --directory /path/to/mcp-video-gen run --extra gcp video-gen
```
